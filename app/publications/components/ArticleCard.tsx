...
export default async function ArticleCard({ id, variant = "internal" }) {

  // Get article based on variant (internal/external) and id
  const {internalPubs, externalPubs} = await getArticles();
  const article = variant === "internal"
    ? internalPubs.find((art) => art.id === id)
    : externalPubs.find((art) => art.id === id);

  if (!article) {
    return <div>Article not found</div>;
  }

  // Get article metadata & slug
  ...
  // If external, use articleLink; if internal, link to internal route
  if (variant === "external") {
    return ( 
