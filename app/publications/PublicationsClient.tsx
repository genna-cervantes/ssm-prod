...
export default function PublicationsClient({ initialExternalArticles, initialInternalArticles }) {
  
  // 1. Search query & Pagination State
  ...
  const { filteredExternal, filteredInternal } = useMemo(() => {
    if (!searchQuery) {
        return { 
            filteredExternal: initialExternalArticles, 
            filteredInternal: initialInternalArticles 
        };
    }
    // 
    ...
    return {
      filteredExternal: initialExternalArticles.filter(filterFn),
      filteredInternal: initialInternalArticles.filter(filterFn),
    };
  }, [searchQuery, initialExternalArticles, initialInternalArticles]);

  // 3. Pagination Logic
  const visibleInternalArticles = filteredInternal.slice(0, visibleCount);

  // 3.1 Search & Pagination Handlers
